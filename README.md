...
Lưu Ý! Đây Là Sản Phẩm Được Dragon Remake ( Chính Bởi Facebook-Chat-Api Của Schmavery, Tác Giả Không Chịu Trách Nghiệm Nào ! ), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
```bash
npm install fca-Tuong@latest
```
Hoặc
```bash
npm i fca-Tuong@latest
```
...
```javascript
const login = require("fca-Tuong"); // lấy từ lib ra 
